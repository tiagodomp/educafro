<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\AdministradoresAco $administradoresAco
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Administradores Aco'), ['action' => 'edit', $administradoresAco->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Administradores Aco'), ['action' => 'delete', $administradoresAco->id], ['confirm' => __('Are you sure you want to delete # {0}?', $administradoresAco->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Administradores Acoes'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Administradores Aco'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Administradores'), ['controller' => 'Administradores', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Administradore'), ['controller' => 'Administradores', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="administradoresAcoes view large-9 medium-8 columns content">
    <h3><?= h($administradoresAco->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Administradore') ?></th>
            <td><?= $administradoresAco->has('administradore') ? $this->Html->link($administradoresAco->administradore->id, ['controller' => 'Administradores', 'action' => 'view', $administradoresAco->administradore->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($administradoresAco->id) ?></td>
        </tr>
    </table>
</div>
