...
<div class="users index large-10 medium-9 columns content">
    <h3><?= __('Codigos') ?></h3>
    <em class="text-right">Foi encontrado <?= echo $countSearch;?> resultados</em>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($query as $codigos): ?>
            <tr>
                <td><?= $this->Number->format($codigos->id) ?></td>
                <td><?= h($codigos->code) ?></td>
                <td><?php if($this->Number->format($codigos->ativacao)>0){echo '<b style="color: red;">Indisponível</b>';} else {echo '<b style="color: green;">Disponível</b>';} ?></td>
                <td><?= h($codigos->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('Ver'), ['action' => 'view', $codigos->id]) ?>
                    <?= $this->Html->link(__('Editar'), ['action' => 'edit', $codigos->id]) ?>
                    <?= $this->Form->postLink(__('Deletar'), ['action' => 'delete', $codigos->id], ['confirm' => __('Você realmente quer deletar o código {0}?', $codigos->code)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
